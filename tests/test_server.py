...
from src.hsi_server.main import (
    get_gemini_client,
    get_hsi_data,
...
class TestFastMCPIntegration:
    ...
    @patch.dict(
        os.environ,
        {"GOOGLE_CLOUD_PROJECT": "test-project", "HOST": "127.0.0.1", "PORT": "9999"},
    )
...
class TestToolFunctions:
    ...
    def test_get_hsi_news_summary_success(self, mock_scraper, mock_get_gemini_client):
        ...
        mock_headlines = [
            {
                "headline": "HSI rises on positive sentiment",
                "url": "http://example.com/1",
            },
            {
                "headline": "Technology stocks gain momentum",
                "url": "http://example.com/2",
            },
        ]
        mock_scraper.get_news_headlines.return_value = mock_headlines
        mock_scraper.get_hsi_data.return_value = {
            "timestamp": "2024-12-06T15:30:00+08:00"
        }

        mock_gemini_client = Mock()
        mock_gemini_client.summarize_headlines.return_value = (
            "Markets show positive trends with technology sector leading gains."
        )
    ...
    @patch("src.hsi_server.main.get_gemini_client")
    @patch("src.hsi_server.main.scraper")
    def test_get_hsi_news_summary_ai_failure(
        self, mock_scraper, mock_get_gemini_client
    ):
...
class TestStockQuoteToolFunction:
    ...
    @patch("src.hsi_server.main.get_gemini_client")
    @patch("src.hsi_server.main.quote_scraper")
    def test_get_stock_quote_with_company_name_success(
        self, mock_quote_scraper, mock_get_gemini_client
    ):
    ...
    def test_get_stock_quote_company_lookup_failure(self, mock_get_gemini_client):
        ...
        assert response_data["success"] is False
        assert (
            "Could not find Hong Kong stock symbol for company: Unknown Company"
            in response_data["error"]
        )
        mock_gemini_client.lookup_stock_symbol.assert_called_once_with(
            "Unknown Company"
        )
...
class TestHSIDataScraper:
    ...
    def test_live_hsi_data_scraping(self):
        ...
        try:
            ...
            print(
                f"✅ Live scraping test passed. HSI: {data['current_point']}, "
...
class TestGeminiClient:
    ...
    def test_gemini_client_initialization_missing_project(self):
        ...
        try:
            ...
            with pytest.raises(
                ValueError,
                match="GOOGLE_CLOUD_PROJECT environment variable is required",
            ):
                GeminiClient()
        finally:
            if original_project:
    ...
    def test_fallback_summary(self):
        ...
        with (
            patch("vertexai.init"),
            patch("src.hsi_server.gemini_client.GenerativeModel"),
        ):
            client = GeminiClient()
            ...
            assert any(
                ...
                for word in ["gain", "positive", "tech", "china"]
            )
